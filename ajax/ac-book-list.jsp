...
<%
  ...
  if(request.getParameter("key") != null){
   String key = request.getParameter("key");
    ps = conn.prepareStatement("select * from books where isbn like ? order by isbn fetch next 10 rows only");
    ps.setString(1,key+"%");
    rs = ps.executeQuery();
    int num_rows = 0;
     //System.out.print(rs);
     ...
     while(rs.next()){
       ...
       jsonObject.put("q", rs.getString("isbn"));
       jsonObject.put("user_search", key);
       jsonObject.put("search_key", rs.getString("name")+" ("+rs.getString("isbn")+")");
